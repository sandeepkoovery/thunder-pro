...
export default function Dashboard({ stats = {}, todayAttendance, recentTasks = [] }) {
  ...
  const [enablingPasskey, setEnablingPasskey] = useState(false);
  ...
  const handleEnablePasskey = async () => {
    ...
    try {
      setEnablingPasskey(true);
      const res = await startPasskeyRegistration("Windows Hello Passkey");
      toast.success(res.message || "Passkey enabled successfully!");
      setPasskeySuccess(true);
    } catch (err) {
      console.error(err);
      const errMsg = err.response?.data?.message || err.message || "Failed to enable passkey.";
      if (!errMsg.includes("cancelled")) {
        toast.error(errMsg);
      }
    } finally {
      setEnablingPasskey(false);
    }
  };

  const isPasskeyEnrolled = auth?.user?.has_passkey || passkeySuccess;
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="mp-vuesy-header text-white -mx-[28px] -mt-[24px] px-[28px] py-6 sm:py-8 shadow-sm transition-all duration-300 relative">
        ...
        >
          ...
          <div className="px-4 md:border-r border-white/10 last:border-none">
            ...
            <div className="text-2xl sm:text-3xl font-bold mt-2 text-cyan-300">{inProgressTasks}</div>
          </div>
          {/* Stat 4: Pending */}
          <div className="px-4 md:border-r border-white/10 last:border-none">
            <div className="text-xs text-purple-300 font-medium uppercase tracking-wider">Pending Tasks</div>
            <div className="text-2xl sm:text-3xl font-bold mt-2 text-amber-300">{pendingTasks}</div>
          </div>
          {/* Stat 5: Pending Leaves */}
          <div className="px-4 md:border-r border-white/10 last:border-none">
            <div className="text-xs text-purple-300 font-medium uppercase tracking-wider">Pending Leaves</div>
            <div className="text-2xl sm:text-3xl font-bold mt-2 text-purple-200">{pendingLeaves}</div>
          </div>
          {/* Stat 6: Success Rate */}
          <div className="px-4 border-none">
            <div className="text-xs text-purple-300 font-medium uppercase tracking-wider">Task Success</div>
            <div className="text-2xl sm:text-3xl font-bold mt-2 text-white">{successRate}%</div>
          </div>
        </div>
      ...
      {/* WINDOWS HELLO ENABLE / ACTIVE PASSKEY BANNER */}
      {isPasskeyEnrolled ? (
        <div className="bg-emerald-50/60 rounded-2xl p-5 shadow-sm border border-emerald-100 flex flex-col md:flex-row items-center justify-between gap-4 transition-all">
          <div className="flex items-center gap-4">
            <div className="p-3 bg-emerald-100/80 text-emerald-700 rounded-xl border border-emerald-200/60 shrink-0">
              <ShieldCheck className="w-7 h-7 text-emerald-600" />
            </div>
            <div>
              <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
                Windows Hello Passkey Active
              ...
              <p className="text-xs text-slate-600 mt-0.5 font-normal">
                Your account is protected with Windows Hello. You can sign in passwordlessly using Fingerprint, Face, or PIN anytime from Chrome or Edge.
              </p>
            </div>
          </div>
          ...
          <Link
            ...
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-emerald-600 hover:bg-emerald-700 text-white font-semibold text-xs sm:text-sm rounded-xl transition-all shadow-sm active:scale-95 shrink-0"
          >
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-indigo-100 flex flex-col md:flex-row items-center justify-between gap-4 transition-all">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
                ...
                <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-xs font-semibold bg-indigo-50 text-indigo-600 border border-indigo-200/80">
                  Passwordless
                </span>
              </h3>
              <p className="text-xs text-slate-500 mt-0.5 font-normal">
                Sign in next time with Fingerprint, Face recognition, or Windows PIN directly from Chrome or Edge—no password required.
              </p>
            </div>
          </div>
          ...
          <button
            ...
            disabled={enablingPasskey}
            className="inline-flex items-center justify-center gap-2 px-5 py-2.5 bg-gradient-to-r from-blue-600 to-indigo-600 hover:from-blue-700 hover:to-indigo-700 text-white font-semibold text-xs sm:text-sm rounded-xl transition-all shadow-md hover:shadow-indigo-500/20 active:scale-95 shrink-0 disabled:opacity-50"
          >
            {enablingPasskey ? (
              <>
