...
export default function UserLayout({ children, title = "Dashboard" }) {
  ...
  const isManagementAdmin = ['admin', 'superadmin'].includes(userRole);
  ...
  const renderNav = ({ collapsed, isMobileOpen }) => {
    const modulesList = [
      ...
      { key: 'drive', order: getModuleOrder('drive', 11), element: <NavItem key="drive" href={route(isManagementAdmin ? "admin.drive.index" : "drive.index")} icon={FolderKanban} label="Drive" routeName={isManagementAdmin ? "admin.drive" : "drive"} visible={isVisible("drive") && !hiddenMenuItems.includes("drive")} beta={false} collapsed={collapsed} isMobileOpen={isMobileOpen} /> },
