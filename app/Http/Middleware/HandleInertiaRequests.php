...
class HandleInertiaRequests extends Middleware
{
    ...
    public function share(Request $request): array
    {
        ...
        if ($user && $user->role === 'superadmin') {
            ...
            if (is_array($rolePermissions) && isset($rolePermissions[$userRoleKey]) && is_array($rolePermissions[$userRoleKey])) {
                ...
                $defaultRolePermissions = [
                    'editor'   => ['dashboard', 'projects', 'attendance', 'leaves', 'calendar', 'content_calendar', 'daily_listings', 'designers_worklist', 'drive', 'chat', 'reports', 'notifications'],
        ...
        return [
            ...
            'userPlan' => $plan,
            'subscriptionStatus' => $admin ? ($admin->isInTrial() ? 'trial' : ($admin->isTrialExpired() ? 'expired' : ($admin->subscription_status ?? 'active'))) : 'active',
            'isTrial' => $admin ? $admin->isInTrial() : false,
